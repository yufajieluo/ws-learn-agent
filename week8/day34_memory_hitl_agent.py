...
def chat_loop():
    ...
    while True:
        ...
        snapshot = app_workflow.get_state(config)
        if snapshot.next:
            pending_action = snapshot.values['messages'][-1]
            tool_name = pending_action.tool_calls[0]['name']

            print(f"\n\n🛑 [安全拦截] AI 申请调用工具: 【{tool_name}】")
            auth = input("👉 是否授权执行？(y/n): ")

            if auth.lower() == 'y':
                print('✅ 授权成功，正在执行...')
                events_resume = app_workflow.stream(
                    None,
                    config = config,
                    stream_mode= 'values'
                )
                print("✨ AI 继续回复: ", end="", flush=True)
